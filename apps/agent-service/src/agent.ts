...
async function extractObjective(openai: OpenAI, request: string): Promise<AgentObjective> {
  const fallback: AgentObjective = { originalRequest: request, currency: 'INR', requiredItems: [], preferences: [] };
  try {
    ...
    return {
      ...
      currency: 'INR',
      requiredItems: Array.isArray(parsed.required_items) ? parsed.required_items.filter((x: unknown) => typeof x === 'string') : [],
      maximumAmount: Number.isInteger(parsed.maximum_amount) ? parsed.maximum_amount : undefined,
...
export async function runAgent(input: { repo: MerchantRepository; message?: string; sessionId?: string; trustedInstruction?: string; }) {
  ...
  try {
    ...
    for (let step = 0; step < 24; step++) {
      ...
      const assistant = completion.choices?.[0]?.message; if (!assistant) throw new Error('LLM returned no message'); messages.push(assistant);
      const calls = assistant.tool_calls || [];
      if (!calls.length) {
        const text = assistant.content || ''; if (text) events.push({ type: 'model', at: nowIso(), text }); input.repo.saveSession(state, messages); return { session_id: state.sessionId, message: text, state, events };
      }
      if (assistant.content) events.push({ type: 'model', at: nowIso(), text: assistant.content });
      for (const call of calls) {
        const toolName = call.function.name; const args = JSON.parse(call.function.arguments || '{}'); events.push({ type: 'tool_call', at: nowIso(), tool: toolName, arguments: args });
        const toolResult = await mcp.callTool({ name: toolName, arguments: args }); const text = mcpText(toolResult); let parsed: any = text; try { parsed = JSON.parse(text); } catch {}
        events.push({ type: 'tool_result', at: nowIso(), tool: toolName, result: parsed }); updateStateFromTool(state, toolName, parsed); events.push({ type: 'state', at: nowIso(), state: structuredClone(state) }); messages.push({ role: 'tool', tool_call_id: call.id, content: text });
        if (toolName === 'commit_quote' && !parsed?.error) {
          ...
          messages.push({ role: 'assistant', content: textOut }); events.push({ type: 'model', at: nowIso(), text: textOut }); input.repo.saveSession(state, messages); return { session_id: state.sessionId, message: textOut, state, events };
        }
      }
