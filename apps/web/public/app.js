...
function showOrder(order) {
  if (!order) return;
  state.order = order; $('orderId').textContent = order.id; $('paymentPanel').hidden = false;
}
...
$('approveButton').addEventListener('click', async () => {
  ...
  try {
    ...
    $('approvalPanel').hidden = true; $('executionPanel').hidden = false;
    toast('Approved. You can mutate merchant state before continuing execution.');
  ...
  const checkout = new Razorpay({
    ...
    handler: async (result) => {
      try {
        ...
        $('paymentPanel').hidden = true; state.task.phase = 'PAYMENT_COMPLETE'; await refreshAudit();
...
$('resetMerchant').addEventListener('click',async()=>{
  try{
    await json(`${adminBase}/reset`,{method:'POST',body:'{}'}); rememberSession(null); state.task=null;state.quote=null;state.order=null;
    $('cartBody').hidden=true;$('cartEmpty').hidden=false;$('approvalPanel').hidden=true;$('executionPanel').hidden=true;$('paymentPanel').hidden=true;
    toast('Merchant and local test state reset.');await loadMerchant();await refreshAudit();
  }catch(e){toast(e.message)}
});
$('newSession').addEventListener('click',()=>{
  rememberSession(null); state.task=null;state.quote=null;state.order=null;
...
async function restoreSession() {
  ...
  try {
    ...
    if (snapshot.payment?.state === 'PAYMENT_SUCCESS') $('paymentPanel').hidden = true;
    addMessage('assistant', `Restored persisted session ${saved}. Current phase: ${snapshot.state.phase}.`);
  } catch {
