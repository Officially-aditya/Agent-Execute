...
function renderGuardIssue(result){
  ...
  if(stale){
    const approved=findNested(result,'approved')||{}; const current=findNested(result,'current')||{};
    const approvedAmount=approved.amount??approved.total??state.quote?.amount; const currentAmount=current.amount??current.total??state.cart?.total;
    const amountChanged=(approvedAmount!=null&&currentAmount!=null&&approvedAmount!==currentAmount)||text.includes('amount updated');
    const title=amountChanged?'Transaction failed because amount updated':'Checkout changed';
    const copy=amountChanged?'Transaction failed because amount updated. The guard stopped execution before payment.':'The merchant changed after your approval. The guard stopped execution before payment.';
    const pill=amountChanged?'AMOUNT UPDATED':'STALE STATE';
    appendCard(`guard:${Date.now()}`,`<div class="card-head"><div class="card-title"><span class="card-icon">!</span>${escapeHtml(title)}</div></div><p class="card-copy">${escapeHtml(copy)}</p><div class="compare-amounts"><div><strong>${approvedAmount!=null?money(approvedAmount):'Approved'}</strong><span>Approved</span></div><span class="compare-arrow">→</span><div><strong>${currentAmount!=null?money(currentAmount):'Changed'}</strong><span>Current</span></div></div><div class="alert-box"><b>No payment was created.</b><br>The agent can now re-read the merchant and recover with a fresh quote.</div><div class="proof-meta"><span>Reason</span><code>${escapeHtml(code)}</code></div>`); return;
  }
  const blocked=['INVALID_SIGNATURE','AMOUNT_MISMATCH','CURRENCY_MISMATCH','MERCHANT_MISMATCH','REPLAY_ATTEMPT','GRANT_ALREADY_USED','PAYMENT_VERIFICATION_FAILED','APPROVAL_EXPIRED','QUOTE_EXPIRED'].includes(code);
  if(blocked){
    const isExpired=code==='APPROVAL_EXPIRED'||code==='QUOTE_EXPIRED';
    const title=isExpired?'Authorization expired':'Execution blocked';
    const copy=isExpired?'The quote or approval expired before payment could be executed.':'The deterministic guard refused to create a payment.';
    appendCard(`blocked:${Date.now()}`,`<div class="card-head"><div class="card-title"><span class="card-icon">×</span>${escapeHtml(title)}</div></div><p class="card-copy">${escapeHtml(copy)}</p><div class="error-box">${escapeHtml(code)}</div><div class="alert-box">No payment was created. The agent cannot bypass this result.</div>`);
  }
}
function showOrder(order){
  if(!order)return; state.order=order; const amount=order.amount??state.quote?.amount??0;
  appendCard(`order:${order.id}`,`<div class="card-head"><div class="card-title"><span class="card-icon">✓</span>Razorpay Order Ready</div></div><div class="amount-hero">${money(amount)}</div><p class="card-copy">The Execution Guard verified quote integrity and created a live Razorpay Test Order. The agent has zero access to Razorpay credentials or payment amounts — the charged amount is locked strictly from your cryptographically signed quote.</p><div class="proof-list"><div><i>✓</i> Real Razorpay Order: <code>${escapeHtml(order.id)}</code></div><div><i>✓</i> Order notes bound to cryptographic quote & grant</div><div><i>✓</i> Exact approved amount locked: ${money(amount)}</div><div><i>✓</i> Official Razorpay Standard Checkout SDK ready</div></div><div class="proof-meta"><span>Razorpay Order</span><code>${escapeHtml(order.id)}</code><span>Gateway Rail</span><code>Razorpay Test API</code><span>Verification</span><code>HMAC SHA-256</code></div><button class="card-action" data-action="checkout" type="button">Pay ${money(amount)} with Razorpay Test Checkout</button>`);
}
function renderPaymentSuccess(paymentId){
  if(state.order)resolveCard(`order:${state.order.id}`); appendCard(`success:${paymentId||state.order?.id||Date.now()}`,`<div class="card-head"><div class="card-title"><span class="card-icon">✓</span>Razorpay Payment Verified</div></div><div class="amount-hero">${money(state.order?.amount??state.quote?.amount??0)}</div><p class="card-copy">Razorpay Checkout completed successfully. Server-side HMAC SHA-256 signature verification confirmed.</p><div class="proof-list"><div><i>✓</i> Razorpay Payment ID: <code>${escapeHtml(paymentId)}</code></div><div><i>✓</i> Bound Razorpay Order: <code>${escapeHtml(state.order?.id)}</code></div><div><i>✓</i> Server HMAC-SHA256 signature verified against Razorpay secret</div><div><i>✓</i> Sealed in immutable database audit log</div></div><div class="proof-meta"><span>Payment ID</span><code>${escapeHtml(paymentId)}</code><span>Order ID</span><code>${escapeHtml(state.order?.id)}</code></div><div class="success-box">✓ The exact approved amount was charged.<br><b>SEEN = COMMITTED = APPROVED = CHARGED</b></div>`); }
function renderPaymentFailure(reason,retry=true){ appendCard(`failure:${Date.now()}`,`<div class="card-head"><div class="card-title"><span class="card-icon">×</span>Payment failed</div></div><p class="card-copy">Razorpay could not complete the test payment. Quote integrity remains separate from this failure.</p><div class="error-box">${escapeHtml(reason||'PAYMENT_FAILED')}</div>${retry?'<button class="card-action red" data-action="checkout" type="button">Try again</button>':''}`); }
...
async function renderTask(task){
  ...
  if(task?.phase==='AWAITING_APPROVAL'&&task.activeQuoteId){
    ...
    const maxBudget=state.task?.objective?.maximumAmount;
    if(maxBudget==null || state.quote.amount <= maxBudget) renderQuoteCard();
  }
...
$('messages').addEventListener('click',async e=>{
  ...
  if(action==='cancel'){
    if(!state.quote)return; const quoteId=state.quote.quoteId; e.target.disabled=true;
    try{
      if(state.sessionId){
        const res=await json(`/api/quotes/${encodeURIComponent(quoteId)}/cancel`,{method:'POST',body:JSON.stringify({session_id:state.sessionId})});
        if(res.state)state.task=res.state;
      }
      resolveCard(`quote:${quoteId}`); state.quote=null; renderSessionMeta(); toast('Cart approval cancelled.'); addMessage('assistant','You cancelled this quote. Let me know what you would like to change or buy instead.'); await refreshAudit();
    }catch(err){toast(err.message);e.target.disabled=false;}
  }
  if(action==='continue'){
...
(async function boot(){
  try{const h=await json('/health');$('healthDot')?.classList.add('ok');const missing=[h.llm_configured?null:'LLM',h.razorpay_test_configured?null:'Razorpay Test'].filter(Boolean);if($('healthText'))$('healthText').textContent=missing.length?`Online · configure ${missing.join(' + ')}`:'Agent + payment ready';}catch{if($('healthText'))$('healthText').textContent='Agent service offline';}
