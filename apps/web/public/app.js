...
async function refreshCart() {
  ...
  try {
    ...
    $('cartItems').innerHTML = cart.items.map(i => `<div class="cart-line"><div><b>${escapeHtml(i.name)}</b><small>${i.productId} · qty ${i.quantity} × ${money(i.unitPrice)}</small></div><b>${money(i.lineTotal)}</b></div>`).join('');
...
async function renderTask(task) {
  ...
  if (task?.phase === 'AWAITING_APPROVAL' && task.activeQuoteId) {
    ...
    $('quoteDigest').textContent = state.quote.cartDigest; $('quoteSignature').textContent = state.quote.signatureValid ? 'VALID' : 'INVALID'; $('quoteExpiry').textContent = new Date(state.quote.validUntil).toLocaleTimeString();
...
async function runAgent(message, continuing = false) {
  ...
  try {
    ...
    state.sessionId = result.session_id; renderEvents(result.events); await renderTask(result.state);
    ...
    if (orderEvent) {
      state.order = orderEvent.result.order; $('orderId').textContent = state.order.id; $('paymentPanel').hidden = false;
    }
    await refreshAudit();
...
$('approveButton').addEventListener('click', async () => {
  if (!state.quote || !state.sessionId) return;
  ...
  try {
    ...
    $('approvalPanel').hidden = true; await runAgent('', true);
...
$('checkoutButton').addEventListener('click', async () => {
  ...
  const checkout = new Razorpay({
    key: state.razorpayKey, order_id: state.order.id, amount: state.order.amount, currency: state.order.currency,
    name: 'Agent Execute', description: 'Verified agent checkout — Test Mode',
    handler: async (result) => {
      try {
        ...
        $('paymentPanel').hidden = true; await refreshAudit();
  ...
  checkout.on('payment.failed', async (response) => {
    await json('/api/payments/failure', { method: 'POST', body: JSON.stringify({ order_id: state.order.id, error: response.error }) }).catch(()=>{});
    addMessage('assistant', `Razorpay Test Checkout failed at the payment rail. Quote integrity remained verified.`); await refreshAudit();
  });
  checkout.open();
});
...
async function refreshAudit() {
  try {
    ...
    for (const event of audit) {
      const row = document.createElement('div'); row.className = 'event'; row.innerHTML = '<span class="kind"></span><span></span><pre></pre>';
...
async function loadMerchant() {
  try {
    ...
    $('products').innerHTML = data.products.map(p => `<div class="product" data-id="${p.id}"><strong>${escapeHtml(p.name)}</strong><small>${p.id} · ${p.category}</small><div class="controls"><input class="price" type="number" min="0" step="1" value="${(p.price/100).toFixed(2)}"><button class="price-btn">Price</button><input class="inventory" type="number" min="0" step="1" value="${p.inventory}"><button class="stock-btn">Stock</button></div></div>`).join('');
...
$('products').addEventListener('click', async (e) => {
  ...
  try {
    ...
    toast('Merchant state changed. The agent has not been told what to do next.'); await refreshCart(); await refreshAudit();
...
$('resetMerchant').addEventListener('click',async()=>{try{await json(`${adminBase}/reset`,{method:'POST',body:'{}'});state.sessionId=null;state.task=null;state.quote=null;state.order=null;$('cartBody').hidden=true;$('cartEmpty').hidden=false;$('approvalPanel').hidden=true;$('paymentPanel').hidden=true;toast('Merchant and local test state reset.');await loadMerchant();await refreshAudit();}catch(e){toast(e.message)}});
$('newSession').addEventListener('click',()=>{state.sessionId=null;state.task=null;state.quote=null;state.order=null;$('approvalPanel').hidden=true;$('paymentPanel').hidden=true;addMessage('assistant','Started a fresh agent session. Existing merchant state is unchanged.');});
...
(async function boot(){
  try { const h=await json('/health'); $('healthDot').classList.add('ok'); $('healthText').textContent=h.ok?'Agent service online':'Agent service issue'; } catch { $('healthText').textContent='Agent service offline'; }
